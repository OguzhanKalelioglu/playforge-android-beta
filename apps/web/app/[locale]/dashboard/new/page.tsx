...
function NewOrderForm({ locale }: { locale: Locale }) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      router.push(`/${locale}/dashboard/orders/${order.id}/pay`)
    } catch (e) {
  ...
  return (
    <div className="container max-w-5xl py-8">
      ...
      <h1 className="mt-3 text-2xl font-semibold tracking-tightish">{t('title')}</h1>
...
export default function NewOrderPage({ params }: { params: Promise<{ locale: Locale }> }) {
  return (
    <Suspense fallback={null}>
      <NewOrderResolver params={params} />
    </Suspense>
  )
}

async function NewOrderResolver({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params
  return <NewOrderForm locale={locale} />
}
